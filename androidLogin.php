...
if($_SERVER['REQUEST_METHOD'] == "POST")
{
	...
	if($txtEmail!="" && $regno!=""){
		$stmt_in = $conn->prepare("SELECT * FROM userDetails where userRegNo=? And password=? Limit 1");
		$stmt_in->execute(array($regno,$txtEmail));
		$affected_rows_in = $stmt_in->rowCount();
		if($affected_rows_in >= 1) 
		{	
			$row = $stmt_in->fetch(PDO::FETCH_ASSOC);
			$_SESSION['app_id'] = $row['userID'];
			$_SESSION['regno'] = $row['userRegNo'];
			$_SESSION['email_id'] = $row['userEmail'];
			...
			<hr/>
				...
				<div class="form-group">
					...
					<div class="input-group">
						...
						<input type="password" onkeypress="wipeboxeror('4')" class="form-control" id="txtEmail" name="txtEmail" required="true" value="<?php echo $txtEmail;?>" placeholder="Enter Your Password" />
					</div>
