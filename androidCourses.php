...
if (isset($_GET['derem'])){
	if($_GET['derem'] !=""){
		$stmt_in = $conn->prepare("DELETE FROM usercourse WHERE id=? and userID=?");
		...
		<div class="col-xs-12 col-sm-12 col-md-4 col-lg-4" style="margin-top:15px;margin-bottom:15px;">
			<div class="col-xs-12">
				<form role="form"  name="reg_form"  id="form" class="form-vertical" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" enctype="multipart/form-data" method="POST">
					<div class="form-group">
						<label for="txtcPermadd">Add More Of Your Best Courses : </label>
						<div class="input-group">
							<span class="input-group-addon"><span class="glyphicon glyphicon-pencil"></span></span>
							<input type="phone" class="form-control" id="txtcPermadd" value="<?php echo $txtcPermadd;?>" name="txtcPermadd" required="true" placeholder="Enter Name Of Your Best Courses"> </input>
						</div>
					</div>
					...
					<div class="form-group pull-right">
						<div class="input-group">
							<input type="submit" name="submit" style="margin-bottom:10px;padding:5px 20px 5px 20px" value="Add Friend To List" class="btn btn-primary btn-md"></input>
						</div>
					</div>
				</form>
			</div>
			<div class="col-xs-12" style="background-color:white;">
				<?php
					...
					$stmt_in = $conn->prepare("SELECT * FROM usercourse where userID=? order by id desc");
					...
					{	
						...
						while($row = $stmt_in->fetch(PDO::FETCH_ASSOC)){
							echo "<tr>
									<td>".$count."</td>
									<td>".$row['bestCourses']."</td>
									<td><a href=androidCourses.php?derem=".$row['id'].">Remove</a></td>
								</tr>";
								$count =$count + 1;
						}
						echo "</tbody></table>";
